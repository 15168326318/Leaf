...
public class SnowflakeMySQLHolder implements SnowflakeHolder {
    ...
    public SnowflakeMySQLHolder(String ip, Integer port, WorkerIdAllocDao dao,String tableName) {
        this.ip = ip;
    ...
    public boolean init() {
        try {
            LeafWorkerIdAlloc workIdAlloc = dao.getOrCreateLeafWorkerId(tableName,ip,port,listenAddress);
            this.workerID = workIdAlloc.getWorkerId();
            //更新workID到本地缓存文件
    ...
    private void ScheduledUploadData() {
        ...
        }, 1L, 600L, TimeUnit.SECONDS);//每10分钟上报数据
    ...
    private void updateNewData() {
        try {
            ...
            dao.updateMaxTimestamp(tableName,workerID, current);
